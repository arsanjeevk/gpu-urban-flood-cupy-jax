...
\newcommand{\jaxsource}{\par\vspace{1mm}{\tiny\color{gray}Background synthesis: S. Borah, \textit{High-Performance Scientific Computing in Python: The JAX Ecosystem} (2026); project implementation evidence.}}
...
% 9
\begin{frame}{8. PyTorch/CUDA: capability and project role}
\begin{columns}[T,onlytextwidth]
\column{0.47\textwidth}
\begin{block}{What PyTorch provides}
\small
An imperative tensor-programming interface, CUDA device execution, automatic differentiation
and a mature ecosystem for scientific machine learning and neural-network development.
\end{block}
\small
Its eager execution model is convenient for inspecting intermediate tensors, while graph
capture and compilation facilities can optimize suitable workloads.

\column{0.49\textwidth}
\begin{block}{Role in this completed experiment}
\small
PyTorch 2.13.0+cu130 was installed and CUDA availability was confirmed on the production
system. It did \textbf{not} execute the reported flood simulations.
\end{block}
\small
The measured backends were CuPy/CUDA and JAX/XLA. Consequently:
\begin{itemize}\tightitem
\item no PyTorch runtime is reported;
\item no PyTorch speed ranking is claimed;
\item PyTorch is discussed only as relevant GPU-computing context.
\end{itemize}
\end{columns}
\evsource
\end{frame}
...
% 10
\begin{frame}{9. JAX: functional programs compiled through XLA}
\begin{columns}[T,onlytextwidth]
\column{0.56\textwidth}
\begin{block}{Pure state transition}
\[
(\bm U^{n+1},\bm D^{n+1})=
\Phi(\bm U^n,\bm D^n;\,\Delta t,\mathcal G,\mathcal P),
\]
where surface state $\bm U$, drainage state $\bm D$, geometry $\mathcal G$ and physical
parameters $\mathcal P$ are explicit inputs and outputs.
\end{block}
\small
JAX traces array operations with known shapes and dtypes, constructs an intermediate graph,
and lowers it through XLA. XLA can fuse compatible operations and generate device code while
JAX preserves an immutable programming interface.

\column{0.40\textwidth}
\centering
\setlength{\fboxsep}{5pt}
\fcolorbox{hfblue}{hflight}{\parbox{0.76\linewidth}{\centering Python array function}}

$\Downarrow$ trace

\fcolorbox{hfblue}{hflight}{\parbox{0.76\linewidth}{\centering JAX computation graph}}

$\Downarrow$ lower and optimize

\fcolorbox{hfblue}{hflight}{\parbox{0.76\linewidth}{\centering XLA executable}}

$\Downarrow$

\fcolorbox{hfgreen}{white}{\parbox{0.76\linewidth}{\centering NVIDIA GPU}}
\end{columns}
\jaxsource
\end{frame}

% 11
\begin{frame}{10. JAX/XLA choices in the flood solver}
\small
\begin{tabularx}{\textwidth}{@{}p{0.24\textwidth}XX@{}}
\toprule
\textbf{JAX mechanism} & \textbf{Use in this project} & \textbf{Scientific implication}\\
\midrule
\texttt{jit} and XLA & Compiles the production update & Compilation is distinguished from steady execution\\
Immutable state & Named state tuples are returned by each update & State evolution is explicit and traceable\\
Scatter reductions & Accumulates edge and network contributions & Matches unstructured-mesh connectivity\\
\texttt{lax.while\_loop} & Keeps adaptive time integration on device & Avoids a Python-controlled inner timestep loop\\
Device synchronization & Waits for completed accelerator work & Prevents asynchronous timing bias\\
\bottomrule
\end{tabularx}

\vspace{3mm}
\begin{alertblock}{Scope actually exercised}
The experiment used one GPU and float32 arrays. Automatic differentiation, \texttt{vmap},
\texttt{pmap}, multi-GPU execution and AI training were not part of the reported production
comparison; no benefit from those features is claimed.
\end{alertblock}
\jaxsource
\end{frame}

% 12
\begin{frame}{11. Verification and experimental protocol}
\begin{columns}[T,onlytextwidth]
\column{0.49\textwidth}
...
% 13
\begin{frame}{12. Production inundation results}
\begin{columns}[T,onlytextwidth]
\column{0.50\textwidth}
...
% 14
\begin{frame}{13. Flood extent, arrival and duration}
\begin{columns}[T,onlytextwidth]
...
% 15
\begin{frame}{14. Velocity, storage and coupled mass response}
\begin{columns}[T,onlytextwidth]
...
% 16
\begin{frame}{15. Cross-backend numerical parity}
\begin{columns}[T,onlytextwidth]
...
% 17
\begin{frame}{16. Repeatability and measured GPU performance}
\begin{columns}[T,onlytextwidth]
...
% 18
\begin{frame}{17. Conclusions and defensible project claims}
...
\vspace{1mm}
{\tiny\color{gray}\url{https://github.com/arsanjeevk/gpu-urban-flood-cupy-jax}}
\end{frame}
